...
def main(args):
    ...
    #print("heigenvalues = " + str(s))

    print("U " + str(U.shape) + ", s " + str(np.diag(s).shape) + ", Vh " + str(Vh.shape))
    #print("urm " + str(user_ratings_mean.reshape(-1, 1).shape))

    print("Reconstructing matrix")

    #print(pd.DataFrame(U).head())
    #print(pd.DataFrame(np.diag(s)).head())
    #print(pd.DataFrame(Vh).head())
    #print(pd.DataFrame(np.dot(np.dot(U, np.diag(s)), Vh)).head())
    print(df.head())
    print(str(user_ratings_mean))
    print(df.add(user_ratings_mean, axis = 1).head())
    #print(pd.DataFrame(np.dot(np.dot(U, np.diag(s)), Vh), columns=df.columns).add(user_ratings_mean, axis = 0).round(decimals=0).head())

    final_df = pd.DataFrame(np.dot(np.dot(U, np.diag(s)), Vh), columns=df.columns)
    print(final_df.head())
    final_df = final_df.add(user_ratings_mean, axis = 1)
    print(final_df.head())
    final_df = final_df.round(decimals=0)
    print(final_df.head())
